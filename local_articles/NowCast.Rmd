...
`mts_monitor` object's values to NowCast values. We provide details on the 
...
In most cases a user will have created a mts_monitor object using one of the 
...
itself. However, note that this may be done when creating a `mts_monitor` object 
in the first place.
...
commonly stored in `mts_monitor` objects.  
...
Omak_ID <- "e5d75388f0cfcbf6_530470013"

# Recipe to create Omak
Omak <-
  # Start with NW Megafires dataset
  NW_Megafires %>%
  # Select the Omak timeseries
...
NowCast values for the Omak `mts_monitor` object. Let's see if we can verify the 
...
of action might be to create a `mts_monitor` object for Omak for 8/25/15:
...
Omak_4_nowcast <- monitor_nowcast(Omak_4)
print(Omak_4_nowcast$data)
```
...
```{r}
Omak_4_nowcast <- monitor_nowcast(Omak_4, includeShortTerm = TRUE)
print(Omak_4_nowcast$data)
```
