...
par(mar = c(3, 4, 6, 2) + .1)
monitor_timeseriesPlot(airsis, addAQI = TRUE, xpd = NA, main = "2021 PM2.5 Values")
par(mar = c(5, 4, 4, 2) + .1)
addAQILegend("topleft", cex = 0.8)

par(mar = c(3, 4, 6, 2) + .1)
monitor_timeseriesPlot(airsis, ylim = c(0, 300), addAQI = TRUE, main = "2021 PM2.5 Values (zoomed in)")
par(mar = c(5, 4, 4, 2) + .1)
addAQILegend("topleft", cex = 0.8)
```
...
AirNow QC uses a high value cutoff of 175 ug/m3 to flag suspect data. This cutoff
may be too low for monitors deployed near wildfires.

Below we see two histograms, one with all of the values below the AirNow cutoff 
of 175 ug/m3 and a second one with all the values above the cutoff.
...
data <- airsis$data[,-1] %>% as.matrix() %>% as.numeric()
hist(data[data >= 175], breaks = seq(175, 2500, by = 75), main = "Histogram of data >= 175 ug/m3")
par(mar = c(5,4,4,2) + .1)
layout(1)
```
...
34 monitors in this time period had hourly PM2.5 values above 175 ug/m3.
...
rate-of-change values above 40 ug/m3/hr in the monitoring data.
