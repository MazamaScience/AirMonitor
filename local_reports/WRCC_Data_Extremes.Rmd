...
scripts. The following plot shows the values of PM2.5 for a three month period when wildfire
smoke was impacting communities.

```{r all_data_timeseries_plot, echo = FALSE}
par(mar = c(3, 4, 6, 2) + .1)
monitor_timeseriesPlot(wrcc, ylim = c(0, 500), addAQI = TRUE, xpd = NA, main = "2021 PM2.5 Values")
par(mar = c(5, 4, 4, 2) + .1)
...
AirNow QC applies uses a high value cutoff of 175 ug/m3 to flag suspect data.
In the case of USFS temporary monitors deployed to wildfires, this cutoff may
be too low.

Below we see two 
histograms, one with all of the values below the AirNow cutoff of 175 ug/m3 and
a second one with all the values above the cutorff.
...
wrcc %>% 
  ...
  monitor_timeseriesPlot(type = 'l', col = 'black', main = "Three monitors with high values")
...
This cutoff may also be too low for monitors deployed to wildfires.

This histograms of rate-of-change for this time period is below. There is a 
...
high rate-of-change and all appear to be valid.
...
The plots in this mini-report make a case for re-evaluating AirNow QC cutoffs
for USFS temporary monitors deployed in wildfire situations.
